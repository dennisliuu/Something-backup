...
class Net:
    def __init__(self):
        pass
    def Save_as_HTML(self,data,file_path,auto_open=0):
        ...
        print("保存完畢")
    def Get(self,url,header_string="",cookie="",SSL_verify=0,timeout=5,proxy_ip=None,session=None):
        ...
        try:
            ...
            if proxy_ip != None:
                ...
                if session is None:
                    ...
                    rs = session.get(url, headers=header_dict, verify=SSL_verify, cookies=cookie, timeout=timeout,
                                    proxies=proxies)
            else:
                if session is None:
                    rs = requests.get(url, headers=header_dict, verify=SSL_verify, cookies=cookie, timeout=timeout)
        ...
        return rs
    def Poster(self,url,data,header_string="",cookie="",SSL_verify=0,timeout=5,proxy_ip=None,session=None):
        header_dict=self.get_header_dict(header_string)
        try:
            ...
            if proxy_ip != None:
                ...
                if session is None:
                    ...
                    rs=session.post(url, data=data,headers=header_dict, verify=SSL_verify, cookies=cookie, timeout=timeout,
                                proxies=proxies)
            else:
                if session is None:
                    rs=requests.post(url, data=data,headers=header_dict, verify=SSL_verify, cookies=cookie, timeout=timeout)
