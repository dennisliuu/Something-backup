...
class Coach:
    ...
    def get_single_page(self, url):
        ...
        data = dict()
        title = dom('h1.product-name-desc').text().replace("'", '')
        if not title:
            return False
        price = (dom('span.pdp.list').eq(0).text().
                            replace('$', '').replace(',', ''))
        images = dom('div.element-wrapper img')
        mainimages = images.eq(0).attr('src')
        otherimages = []
        for i in range(1, len(images)):
            otherimages.append(images.eq(i).attr('src'))

        color = []
        color_dom = dom('ul.swatches.swatch-list img')
        for i in range(0, len(color_dom)):
            color.append(color_dom.eq(i).attr('alt'))

        size_dom = dom('select#control-select-size option')
        size = []
        if size_dom:
            for i in range(1, len(size_dom)):
                size.append(size_dom.eq(i).text())
        else:
            size = -1
        content = (dom('p.editor-notes').text().replace("'", '')
...
if __name__ == "__main__":
    test = Coach()
    data = test.get_single_page('https://www.coach.com/coach-central-shopper-tote/78217.html?cgid=women-new-arrivals-new-arrivals&dwvar_color=GMPE4')
