...
class WUI():
    ...
    def Add_Recv_Msg_Hook(self,msg_txt,callback,send_msg_to_fn=1,fallback=None):
        self.Add_WS_Hook('rec',msg_txt,callback,send_msg_to_fn,fallback)
    ...
    def Add_WS_Hook(self,hook_type,msg_txt,callback,send_msg_to_fn=1,fallback=None):
        try:
            mb = dict()
            mb['txt'] = msg_txt
            mb['callback'] = callback
            mb['fallback'] = fallback
                ...
                if hook_type == 'rec':
                    if mb['send_msg_to_fn'] == 1:
                        try:
                            ...
                            rb = base64.b64decode(message).decode()
                            # print(rb)
                            # print(type(message))

                            msg_o = json.loads(rb,encoding="utf-8")
                            # print("msg:{}".format(msg_o))
                            order = msg_o['order']
                            detail = msg_o['detail']
                            if order == mb['txt']:
                                mb['callback'](client, detail,mb['fallback'])
